...
int main() {
    ...
    double average = calculateAverage(testScores + 1, numScores - 1);

    // Display the sorted list of scores
    std::cout << "\nSorted Test Scores (lowest score dropped):\n";
    for (int i = 1; i < numScores; i++) {
        std::cout << testScores[i] << " ";
    }

    // Display the average score
    std::cout << "\nAverage Test Score (lowest score dropped): " << std::fixed << std::setprecision(2) << average << std::endl;
