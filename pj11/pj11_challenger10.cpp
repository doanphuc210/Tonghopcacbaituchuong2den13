...
void searchByTopic(const Speaker speakers[], int numSpeakers, const string &keyword);
...
int main()
{
    ...
    do
    {
        ...
        case '4':
            {
                string keyword;
                cout << "Enter the keyword to search for a speaker by topic: ";
                cin.ignore(); // Consume the newline character left in the buffer
                getline(cin, keyword);
                searchByTopic(speakers, numSpeakers, keyword);
            }
            break;
        case '5':
...
void displayAllSpeakers(const Speaker speakers[], int numSpeakers)
{
    if (numSpeakers > 0)
    {
        ...
        {
            cout << setw(4) << i + 1 << setw(20) << speakers[i].name << setw
